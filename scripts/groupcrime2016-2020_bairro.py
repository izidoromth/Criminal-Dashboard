...
df_fraud = df_fraud.groupby(by=['data_fato','descBairro','natureza']).sum().reset_index()
...
df_robbery = df_robbery.groupby(by=['data_fato','descBairro','natureza']).sum().reset_index()
...
df_drug = df_drug.groupby(by=['data_fato','descBairro','natureza']).sum().reset_index()
...
df_dome_viol = df_dome_viol.groupby(by=['data_fato','descBairro','natureza']).sum().reset_index()
...
df_all_crimes.to_csv('./data/pcpr/crimes2016-2020_bairro.csv', index=False)